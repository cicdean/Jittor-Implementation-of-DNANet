...
class ROCMetric():
    ...
    def update(self, preds, labels):
        for iBin in range(self.bins+1):
            ...
            i_tp, i_pos, i_fp, i_neg,i_class_pos = cal_tp_pos_fp_neg(preds, labels, self.nclass,score_thresh)   #class_pos：预测为真的像素数
...
class mIoU():
    ...
    def update(self, preds, labels):
        ...
        correct, labeled = batch_pix_accuracy(preds, labels)    #correct:正确预测的像素数；labeled:标记为正的像素数
        inter, union = batch_intersection_union(preds, labels, self.nclass) #inter:交集的面积;union:并集的面积
...
def batch_intersection_union(output, target, nclass):
    ...
    area_inter, _  = np.histogram(intersection.cpu(), bins=nbins, range=(mini, maxi))   #相交的面积
    area_pred,  _  = np.histogram(predict.cpu(), bins=nbins, range=(mini, maxi))    #预测的面积
    area_lab,   _  = np.histogram(target.cpu(), bins=nbins, range=(mini, maxi)) #标记的面积
